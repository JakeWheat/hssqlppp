...
-- | Version of type check which can type check an individual
-- statement with ? or positional arg placeholders in it. Will error
-- if the statement isn't select, update, insert or delete. For use in
-- type checking embedded parameterized statements.
...
getPlaceholders st =
    filter isPlaceholder $ everything (++) (mkQ [] ga) st
    where
       ...
       _lhsIlib ->
         (let _lhsOannotatedTree :: CopySource
              ...
              _annotatedTree =
                  {-# LINE 57 "./TypeChecking/TypeChecking.ag" #-}
                  Stdin
                  {-# LINE 2032 "AstInternal.hs" #-}
              -- self rule
              _originalTree =
                  {-# LINE 63 "./TypeChecking/TypeChecking.ag" #-}
                  Stdin
                  {-# LINE 2037 "AstInternal.hs" #-}
              -- self rule
              _lhsOannotatedTree =
                  {-# LINE 57 "./TypeChecking/TypeChecking.ag" #-}
                  _annotatedTree
       ...
       _lhsIlib ->
         (let _lhsOannotatedTree :: Statement
              ...
              _statementType =
                  ...
                  [StatementType ph $ leftToEmpty id  $
                                      unwrapSetOfComposite $
                                      getTypeAnnotation _exIannotatedTree]
                  where
                    ph = flip map (getPlaceholders _exIannotatedTree)
                           $ const $ ScalarType "text"
