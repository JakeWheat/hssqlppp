...
Doesn't do namespaces, autogenerated function casts, variadic args or
defaults args
...
type ProtArgCastList = [ProtArgCast]
...
findCallMatch sp f inArgs =
    let icl = initialCandList
        ...
        candCastPairs :: ProtArgCastList
        candCastPairs = zip icl castPairs
        exactMatch :: ProtArgCastList
        exactMatch = getExactMatch candCastPairs
        reachable :: ProtArgCastList
        ...
        filteredForPreferred :: ProtArgCastList
    ...
    where
      ...
      filterCandCastPairs :: ([ArgCastFlavour] -> Bool)
                          -> ProtArgCastList
                          -> ProtArgCastList
      filterCandCastPairs pred ccp = filter (\(_,cp) -> pred cp) ccp

      getFnArgs :: FunctionPrototype -> [Type]
      getFnArgs (_,a,_) = a
      getHeadFn :: ProtArgCastList -> FunctionPrototype
...
{-
      countCastsToPreferred cands =
          flip map cands (\(_,fnArgs,_) ->
-}

{-
      checkCandListCont cands cont =
          case length cands of
            0 -> Left $ TypeError sp (NoMatchingOperator f inArgs)
            1 -> Right $ head cands
            _ -> cont
      checkCandListForOne cands =
          case length cands of
            0 -> Left $ TypeError sp (NoMatchingOperator f inArgs)
            1 -> Right $ head cands
            _ -> Left $ TypeError sp (MultipleMatchingOperators f inArgs)
-}


{-
      filterCannotReach cands =
          let filteredCands = flip filter cands (\(_,candArgs,_) -> checkImplicits inArgs candArgs)
          in checkCandListForOne filteredCands
          where
            checkImplicits (ia:ias) (ca:cas) = if ia == ca || implicitlyCastableFromTo ia ca
                                                 then checkImplicits ias cas
                                                 else False
            checkImplicits [] [] = True
            checkImplicits _ _ = False
-}
